...
curl -sS -k --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" "${VAULT_ADDR}/v1/auth/token/renew-self" | grep -q 'lease_duration'
retval=$?
if [[ $retval -ne 0 ]]; then
...
# NEWPASS=$(curl -sS -k  --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" -H "Content-Type: application/json" --data '{"words":"4","separator":"-"}'  ${VAULT_ADDR}/v1/gen/passphrase | grep -Po '"value":.*?[^\\]"' | awk -F ':' '{print $2}' | tr -d '"')

# Fetch a new password from Vault
#NEWPASS=$(curl -sS -k --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" -H "Content-Type: application/json" --data '{"length":"36","symbols":"0"}'  ${VAULT_ADDR}/v1/gen/password | grep -Po '"value":.*?[^\\]"' | awk -F ':' '{print $2}' | tr -d '"')
...
curl -sS -k --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" --data "$JSON" "${VAULT_ADDR}/v1/systemcreds/data/linux/$(hostname)/${USERNAME}_creds" | grep -q 'request_id'
retval=$?
if [[ $retval -eq 0 ]]; then
